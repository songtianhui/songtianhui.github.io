...
本章开始讲卷积神经网络。这到了比较熟悉的一部分内容，我也会省略一些部分，但有些概念及背后原理老师还是阐释得比较清楚。

## padding

在周围填充像素。

- **Valid**：不填充。输出 $(n - f + 1) \times (n - f + 1)$
- **Same**：填充到输入和输出大小一样。

$f$ 通常是奇数，可以有一个中心点，填充也会对称。

## 卷积步长 Stride

$n \times n$ 的输入，$f \times f$ 的过滤器，$p $ 的 padding，$s$ 的步长，输出大小为 $\left \lfloor\dfrac{n + 2p - f}{s} \right \rfloor \times \left \lfloor\dfrac{n + 2p - f}{s} \right \rfloor$。

*其实卷积操作真正的名字叫互相关，卷积还包含一个反转操作，不过在神经网络中不重要。*



## 三维卷积

总结就是一个$n \times n \times n_{c}$（通道数）的输入图像，卷积层必然是 $f \times f \times n_c$（也就是在编程中只需要指定 $f$，框架会自动pad出通道数）。然后就得到了$（n-f+1）×（n-f+1）×n_{c^{'}}$，这里$n_{c^{'}}$其实就是下一层的通道数，就是你用的过滤器的个数。
...
一个有意思的特点就是，它有一组超参数，但并没有参数需要学习。实际上，梯度下降没有什么可学的，一旦确定了$f$和$s$，它就是一个固定运算，梯度下降无需改变任何值。

池化的超参数包括过滤器大小 $f$ 和步幅 $s$，很少用 padding。最大池化的输入就是$n_{H} \times n_{W} \times n_{c}$，假设没有padding，则输出$\lfloor\frac{n_{H} - f}{s} +1\rfloor \times \lfloor\frac{n_{w} - f}{s} + 1\rfloor \times n_{c}$。



## 为什么使用卷积

他可以通过**共享参数**和**稀疏连接**来减少参数，以便用更小的训练集来训练它，从而预防过度拟合。
...
非常非常深的神经网络是很难训练的，因为存在梯度消失和梯度爆炸问题。跳跃连接（**Skip connection**），它可以从某一层网络层获取激活，然后迅速反馈给另外一层，甚至是神经网络的更深层。

ResNet 是由 **残差块（Residuak Block）**构建的，就是在后面一层的激活函数中的参数加上一个前面层的激活项 $a^{\left\lbrack l + 2 \right\rbrack} = g\left(z^{\left\lbrack l + 2 \right\rbrack} + a^{[l]}\right)$。也就是一个“捷径“，或者“跳跃连接”，就是指$a^{[l]}$跳过一层或者好几层，从而将信息传递到神经网络的更深层。

> 如果我们使用标准优化算法训练一个普通网络，比如说梯度下降法，或者其它热门的优化算法。如果没有残差，没有这些捷径或者跳跃连接，凭经验你会发现随着网络深度的加深，训练错误会先减少，然后增多。而理论上，随着网络深度的加深，应该训练得越来越好才对。也就是说，理论上网络深度越深越好。但实际上，如果没有残差网络，对于一个普通网络来说，深度越深意味着用优化算法越难训练。实际上，随着网络深度的加深，训练错误会越来越多。
>
> 但有了**ResNets**就不一样了，即使网络再深，训练的表现却不错，比如说训练误差减少，就算是训练深达100层的网络也不例外。但是对$x$的激活，或者这些中间的激活能够到达网络的更深层。这种方式确实有助于解决梯度消失和梯度爆炸问题，让我们在训练更深网络的同时，又能保证良好的性能。也许从另外一个角度来看，随着网络越来深，网络连接会变得臃肿，但是**ResNet**确实在训练深度网络方面非常有效。



## 残差网络为什么有用

大概意思就是如果我们在一个 Big NN 后面再加两层，但是 ResNet 可以很好地学到 $a^{[l+2]} = a^{[l]}$ 恒等函数，就是说给残差网络增加层数并不会导致它的效率逊色于更简单的神经网络，所以理论上就是越深越好啦。

还有一个细节是，残差块需要保证 $z^{l+2}$ 和 $a^{[l]}$ 的维度相等，所以需要比较多的 same 卷积。当维度不相同时，就需要一个参数矩阵 $W_s$ 来调整 $a^{[l]}$ 的维度。
...
就是用各种形状的过滤器和池化层，same 卷积叠起来。

基本思想是**Inception**网络不需要人为决定使用哪个过滤器或者是否需要池化，而是由网络自行确定这些参数，可以给网络添加这些参数的所有可能值，然后把这些输出连接起来，让网络自己学习它需要什么样的参数，采用哪些过滤器组合。

一个最重要的问题就是计算成本，参数太庞大了。我们就要通过 $1 \times 1$ 卷积层来缩小网络（瓶颈层），可以再在之后扩大网络，从而大幅度降低了参数和计算成本。
...
用别人训练好的参数，我们就可以不用从头开始训练或者只要训练后部分很浅层的网络。



## 数据增强 Data augmentation

当下在计算机视觉方面，计算机视觉的主要问题是没有办法得到充足的数据。对大多数机器学习应用，这不是问题，但是对计算机视觉，数据就远远不够。所以这就意味着当训练计算机视觉模型的时候，数据扩充会有所帮助。
...
本章主要介绍目标检测（Object detection）的内容。

## 目标定位 Object localization

在图像分类问题中，除了分类之外还要确定对象的位置，就让神经网络再多输出4个数字，$b_x, b_y, b_h, b_w$

，就是对象边界框的中心点坐标和高宽。

对于网络的输出就可以定义为 $y= \ \begin{bmatrix} p_{c} \\ b_{x} \\ b_{y} \\ b_{h} \\ b_{w} \\ c_{1} \\ c_{2}\\ c_{3} \\\end{bmatrix}$，$p_c$ 表示是否含有对象，同时$c_1, c_2, c_3$ 输出分类，损失函数 $L\left(\hat{y},y \right) = \left( \hat{y_1} - y_{1} \right)^{2} + \left(\hat{y_2} - y_{2}\right)^{2} + \ldots\left( \hat{y_8} - y_{8}\right)^{2}$。如果图中没有对象 $p_c = 0$ 则其他参数就都没有意义，损失函数 $\left(\hat{y_1} - y_{1}\right)^{2}$。



## 特征点检测 Landmark detection

运用上面的思想我们就可以检测更多的特征点，比如人脸特征，人体姿态等，用 $(l_{1x}, l_{1y}), (l_{2x}, l_{2y})...$ 等输出各个特征点然后进行回归。



## 目标检测 Object detection

滑动窗口的思想，使用不同大小的窗口，遍历整个图片，每个窗口输入网络预测。
...
首先要知道如何把神经网络的全连接层转化为神经层。就是通过维度一样的卷积层，过滤器的个数是全连接单元的个数，就把全连接转化成了和单元相同个数的通道数的层。再往后的全连接层就都只要 $1\times 1$ 卷积。

然后我们就可以用卷积实现滑动窗口。其实就是把卷积网络的输入作为每一个窗口，按照步长滑动就可以了，最后会得到一个窗口总数也就是各个窗口的输出。我们就可以在并行计算的基础上一次得到所有窗口的输出。

卷积层上的滑动窗口提高了整个算法的效率。不过这种算法仍然存在一个缺点，就是边界框的位置可能不够准确。
...
交并比（**loU**）函数做的是计算两个边界框交集和并集之比。一般约定，在计算机检测任务中，如果$loU≥0.5$，就说检测正确，如果预测器和实际边界框完美重叠，**loU**就是1，因为交集就等于并集。

人们定义**loU**这个概念是为了评价你的对象定位算法是否精准，但更一般地说，**loU**衡量了两个边界框重叠地相对大小。



## 非极大值抑制 Non-max supression

到目前为止学到的对象检测中的一个问题是，算法可能对同一个对象做出多次检测，所以算法不是对某个对象检测出一次，而是检测出多次。非极大值抑制这个方法可以确保你的算法对每个对象只检测一次。

总之就是每个窗口每个检测结果相关的概率 $p_c$，首先看概率最高的那个，这么做之后，非极大值抑制就会逐一审视剩下的矩形，所有和这个最大的边框有很高交并比，高度重叠的其他边界框，那么这些输出就会被抑制。
...
**Regional CNN(R-CNN)**。这个算法尝试选出一些区域，在这些区域上运行卷积网络分类器是有意义的，所以这里不再针对每个滑动窗运行检测算法，而是只选择一些窗口，在少数窗口上运行卷积网络分类器。

选出候选区域的方法是运行图像分割算法，找出色块（像素语义）。

太慢了，有 **Fast R-CNN, Faster R-CNN**。
...
$x^{\langle t \rangle}$：序列的中间位置，$t$ 索引。
...
简单来说，在时间步 $t$ 中，模型不仅是用 $x^{\langle t \rangle}$ 来预测 $\hat{y}^{\langle t \rangle }$，而且还用了前一个时间步中的信息，也就是时间步 $t-1$ 的激活值会传递到 $t$。

有一个零时刻的初始激活值 $a^{\langle 0 \rangle} = \mathbf{0}$。

循环神经网络是从左向右扫描数据，同时每个时间步的参数也是共享的。$W_{\text{ax}}$ 表示管理着从 $x^{\langle t \rangle}$ 到隐藏层的连接的一系列参数，每个时间步使用的都是相同的参数 $W_{\text{ax}}$。而激活值也就是水平联系是由参数 $W_{aa}$ 决定的，同时每一个时间步都使用相同的参数 $W_{aa}$，同样的输出结果由 $W_{\text{ya}}$ 决定。
...
$a^{\langle t \rangle} = g_{1}(W_{aa}a^{\langle t - 1 \rangle} + W_{ax}x^{\langle t \rangle} + b_{a})$

$\hat y^{\langle t \rangle} = g_{2}(W_{ya}a^{\langle t \rangle} + b_{y})$

循环神经网络用的激活函数经常是**tanh**。

简化符号：

$a^{\langle t \rangle} =g(W_{a}\left\lbrack a^{\langle t-1 \rangle},x^{\langle t \rangle} \right\rbrack +b_{a})$

$\hat y^{\langle t \rangle} = g(W_{y}a^{\langle t \rangle} +b_{y})$

所以我们定义$W_{a}$的方式是将矩阵$W_{aa}$和矩阵$W_{ax}$水平并列放置，$[ W_{aa}\vdots W_{ax}]=W_{a}$。

用这个符号（$\left\lbrack a^{\langle t - 1 \rangle},x^{\langle  t \rangle }\right\rbrack$）的意思是将这两个向量堆在一起，用这个符号表示，即$\begin{bmatrix}a^{\langle  t-1 \rangle } \\ x^{\langle  t \rangle } \\\end{bmatrix}$。
...
只要正确地设置了遗忘门和更新门，**LSTM**是相当容易把$c^{\langle 0>}$的值一直往下传递到右边。这就是为什么**LSTM**和**GRU**非常擅长于长时间记忆某个值，对于存在记忆细胞中的某个值，即使经过很长很长的时间步。
...
会有 $\hat y^{\langle t \rangle} =g(W_{g}\left\lbrack \overrightarrow{a}^{\langle  t \rangle },\overleftarrow{a}^{\langle  t \rangle } \right\rbrack +b_{y})$。
